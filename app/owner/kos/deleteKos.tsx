...
const DeleteKos = ({ selectedKos }: {selectedKos: IKos}) => {
    ...
    const handleSubmit = async ( e: FormEvent ) => {
        try {
            e.preventDefault()
            const url = `${BASE_API_URL}/kos/delete/${selectedKos.idKos}`
    ...
    return (
        <div>
            <button 
                type="button" 
                onClick={() => openModal()}
                className="w-full py-2.5 flex items-center justify-center gap-2 bg-red-500 text-white rounded-xl hover:bg-red-600 transition"
            >
